...
const CreateConditionsButton = ({ gatedConditions = [] }: Props) => {
  ...
  const events = useEvent(ArbiGates__factory, 'Created', {
    // args: [null, address],
    onChange: (data) => {
      if (data[1] == address) {
        setGateId(data[0]);
  ...
  const { write, isLoading } = useContractWrite(ArbiGates__factory, 'add', {
    reckless: true,
  });
        ...
        >
          <Text size="sm" style={{ display: 'flex' }}>
            <div style={{ paddingTop: 1 }}>
              <CopyButton
                value={
                  'https://arbigates/api/verify?address=${address}&gateId=' +
                  gateId.toString()
                }
              >
                {({ copied, copy }) => (
